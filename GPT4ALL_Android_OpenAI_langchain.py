...
class TerminalApp(App):
    ...
    def execute_command(self, instance):
        ...
        if hasattr(self, 'process') and self.process.poll() is None:
            ...
            output = ''
            try:
                self.process = subprocess.Popen(['python', 'AppGPT4ALL_langchain.py'], stdin=subprocess.PIPE, stdout=subprocess.PIPE,
                                                stderr=subprocess.PIPE, universal_newlines=True)
                self.process.stdin.write(command + '\n')
                self.process.stdin.flush()
                output, errors = self.process.communicate()
            except FileNotFoundError:
                output = f"Error: Program 'AppGPT4ALL_langchain.py' not found."

            self.output_label.text += '\n$ python AppGPT4ALL_langchain.py\n' + output
